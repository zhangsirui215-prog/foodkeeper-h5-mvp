...
function ingredientEmoji(item) {
  return item.category === '水果' ? '🍌' : item.category === '蔬菜' ? '🥬' : item.category === '肉类' ? '🥩' : item.category === '蛋奶' ? '🥛' : '🍽️';
}

function bodyConditionFromSummary(summary) {
  if (!summary) return null;
  if (summary.workout.includes('HIIT')) {
    return {
      title: '高强度后恢复期',
      desc: '身体处于高消耗状态，优先补水、电解质、易消化碳水和适量蛋白。'
    };
  }
  if (summary.workout.includes('力量')) {
    return {
      title: '力量训练后修复期',
      desc: '肌肉修复需求较高，采购应偏向高蛋白、低脂和复合碳水。'
    };
  }
  if (summary.workout.includes('跑步')) {
    return {
      title: '有氧运动后补能期',
      desc: '糖原和水分消耗明显，建议补充碳水、钾和发酵乳制品。'
    };
  }
  return {
    title: '轻活动后平衡期',
    desc: '身体负担较轻，采购应保持清淡、多蔬果和适量蛋白。'
  };
}
...
function renderHome() {
  ...
  const bodyCondition = bodyConditionFromSummary(summary);
  const shopping = summary ? smartShoppingTips() : [];

  return `
    <main class="page">
      <section class="hero priority-hero card">
        <div class="section-title">
          <div>
            <p class="eyebrow">AI 食材管家 MVP</p>
            <h1>今天优先处理 ${expiring.length} 个临期食材</h1>
            <p class="muted">新版工作台：临期食材已直接展示为可点击图标。</p>
          </div>
          <div class="home-impact-icons" aria-label="环保与成就入口">
            <button class="impact-icon-button" data-page="profile" title="到我的查看碳足迹厨房" aria-label="到我的查看碳足迹厨房">
              <span>🌍</span>
            </button>
            <button class="impact-icon-button" data-page="profile" title="到我的查看成就徽章" aria-label="到我的查看成就徽章">
              <span>🏅</span>
            </button>
          </div>
        </div>
        <div class="priority-food-strip">
          ${expiring.length ? expiring.slice(0, 5).map((item) => `
            <button class="priority-food" data-open-ingredient="${item.id}" title="${item.name}">
              <span class="priority-food-icon">${ingredientEmoji(item)}</span>
              <strong>${item.name}</strong>
              <small>${item.days >= 0 ? `${item.days}天` : '过期'}</small>
            </button>
          `).join('') : '<div class="empty">暂无临期食材</div>'}
        </div>
        <div class="priority-action-row">
          <p>优先消耗：${mustEat}。系统会优先把这些食材放进今日菜谱，减少浪费。</p>
          <button class="primary" data-page="recipes">生成菜单</button>
        </div>
      </section>
      ...
      <section class="card card-section">
        <div class="section-title">
          <h2>运动恢复采购建议</h2>
          <span class="muted">先生成状态，再给建议</span>
        </div>
        ${summary ? `
          <div class="health-shopping-status">
            <article class="status-card card">
              <span>🏃</span>
              <div>
                <strong>${summary.workout} · ${summary.duration}分钟</strong>
                <p class="muted">消耗约 ${summary.kcal} kcal，平均心率 ${summary.avgHr}。</p>
              </div>
            </article>
            <article class="status-card card">
              <span>🩺</span>
              <div>
                <strong>${bodyCondition.title}</strong>
                <p class="muted">${bodyCondition.desc}</p>
              </div>
            </article>
          </div>
          <button class="secondary full-width" data-action="generate-health-shopping">重新模拟生成运动与身体情况</button>
          <div class="stack">
            ${shopping.map((tip) => `
        ...
        ` : `
          <div class="empty">还没有运动记录和身体情况。先模拟生成一次状态，再给出采购建议。</div>
          <button class="primary full-width" data-action="generate-health-shopping">模拟生成运动与身体情况</button>
        `}
      </section>
    </main>
  `;
...
function bindEvents() {
  ...
  document.querySelector('[data-action="generate-health-shopping"]')?.addEventListener('click', () => {
    const scenarioKeys = Object.keys(healthScenarios);
    const scenario = scenarioKeys[Math.floor(Math.random() * scenarioKeys.length)];
    writeJson(store.health, { ...health(), authorized: true, scenario });
    syncHealth();
    showToast('已模拟生成运动记录与身体情况', 'success');
  });
